...
- (void)registerObserver:(id<ALArrayModelObserver>)observer {
    NSMutableSet *mutableObserverSet = self.mutableObserverSet;
    @synchronized(mutableObserverSet) {
...
- (void)unregisterObserver:(id<ALArrayModelObserver>)observer {
    NSMutableSet *mutableObserverSet = self.mutableObserverSet;
    @synchronized(mutableObserverSet) {
...
- (void)notifyObserversWithSelector:(SEL)selector withObject:(id)object {
    ...
    @synchronized(mutableObserverSet) {
        for (ALWeakLink *weakLink in self.mutableObserverSet) {
            id<ALArrayModelObserver> observer = weakLink.target;
